...
public class ActiveRegion implements HasGenomeLocation, Serializable {
     ...
     */
    private transient List<GATKSAMRecord> reads = new ArrayList<GATKSAMRecord>();   // 无法序列化，先暂存起来，再放回去
     ...
     */
    private transient GenomeLocParser genomeLocParser;  // 无法序列化，设定为统一的parser
     ...
     */
    public byte[] getActiveRegionReference(final RefContentProvider refContentProvider) {
        return getActiveRegionReference(refContentProvider, 0);
    }
     ...
     * bases
     *
     * @param refContentProvider the source of the reference genome bases
     ...
     */
    public byte[] getActiveRegionReference(final RefContentProvider refContentProvider, final int padding) {
        return getReference(refContentProvider, padding, extendedLoc);
    }

    /**
     * See #getActiveRegionReference but using the span including regions not the extended span
     */
    public byte[] getFullReference(final RefContentProvider refContentProvider) {
        return getFullReference(refContentProvider, 0);
    }

    /**
     * See #getActiveRegionReference but using the span including regions not the extended span
     */
    public byte[] getFullReference(final RefContentProvider refContentProvider, final int padding) {
        return getReference(refContentProvider, padding, spanIncludingReads);
    }
     ...
     * @return a non-null array of bytes holding the reference bases in referenceReader
     */
    public byte[] getReference(final RefContentProvider refContentProvider, final int padding, final GenomeLoc genomeLoc) {
        if (refContentProvider == null)
        ...
        byte[] reference = refContentProvider.getReferenceContext(genomeLoc, padding).getBases();
    ...
    public void setReads(List<GATKSAMRecord> reads) {
        this.reads = reads;
    }
