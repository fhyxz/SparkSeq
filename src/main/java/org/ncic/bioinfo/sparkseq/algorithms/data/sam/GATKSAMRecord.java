...
public class GATKSAMRecord extends SAMRecord implements Cloneable{
    ...
    // from old BamRecord
    private short mReadNameLength;
    private int mCigarLength;
    ...
    public GATKSAMRecord(final SAMFileHeader header,
                         ...
                         final int indexingBin, //暂时没法用到，因为访问权限问题。不写也没啥关系
                         final int cigarLen,
                         final int flags,
                         final int readLen,
        ...
        super(header);
        setReferenceIndex(referenceSequenceIndex);
        ...
        setFlags(flags);
        mReadLength = readLen;
        setMateReferenceIndex(mateReferenceSequenceIndex);
        setMateAlignmentStart(mateAlignmentStart);
        setInferredInsertSize(insertSize);
        mRestOfBinaryData = variableLengthBlock;
